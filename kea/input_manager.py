...
class InputManager(object):
    ...
    def get_input_policy(self, device, app, master):
        if self.policy_name == POLICY_NONE:
            ...
            input_policy = LLMPolicy(device, app, kea=self.kea, number_of_events_that_restart_app = self.number_of_events_that_restart_app, clear_and_reinstall_app= not self.is_package, allow_to_generate_utg = self.generate_utg, output_dir=self.output_dir,base_url=self.base_url)
        else:
            self.logger.warning(
